...
function date_isfuture(now,d, m, y) {
    //pass today (server date)
...
function date_ispresent(now, d, m, y)
{
   //pass today (server date) 
  if (d.substring(0,1) == '0')
        d = d.substring(1,2);
...
function date_ispast(now, d, m, y) {
    //pass today (server date) 
    if (d.substring(0,1) == '0')
        d = d.substring(1,2);
...
{
    if (msg == null)
        msg = "Are you sure you want to update status?";
	conf = confirm(msg);
	if (conf)
		return true;
	else
